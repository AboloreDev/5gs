...
const EditModal = ({ appointment, onSave, onClose }) => {
  const isoDate = appointment?.date?.split(" ")[0];
  const isoTime = appointment?.date?.split(" ")[1]?.slice(0, 5);

  const [newDate, setNewDate] = useState(isoDate || "");
  const [newTime, setNewTime] = useState(isoTime || "");
  const [reason, setReason] = useState(appointment?.reason || "");

  const handleSave = () => {
    if (newDate && newTime) {
      const combinedDateTime = `${newDate} ${newTime}:00`;
  ...
  return (
    <div className="fixed w-full inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md sm:max-w-lg md:max-w-xl space-y-4">
        ...
        <p>Editing Appointment: {appointment?._id}</p>
        <div>
